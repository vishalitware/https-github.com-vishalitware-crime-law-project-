...
ps = PorterStemmer()
legal_terms_stems = []
legal_terms_stems = [ps.stem(w) for w in legal_terms_stems]		#stemming legal words
legal_terms = legal_terms +legal_terms_stems
# print(len(idf))
idf   = {k:k for (k,v) in idf.items() if v>3}  #remove low idf words , threshould>3
idf   = {k:k for (k,v) in idf.items() if k in legal_terms}
...
result1.to_csv('legal_terms_dtm1.csv',index=False)
...
result2.to_csv('legal_terms_dtm2.csv',index=True)

# documents = df['content']
# documents = documents.apply(lambda s:re.sub(r"[_()\"#/@;:<>(){}```+=~|.!?,|]", " ", s))
...
# # print(cosine_similarity(tfidf_matrix[:], tfidf_matrix))      #the pairwise document similarity
